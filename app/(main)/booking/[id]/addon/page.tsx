...
export default function BookingAddon() {
    ...
    const calculateTotal = () => {
        const addonsTotal = addonQuantities.reduce((sum, item) => 
                                ...
                                <div className="flex flex-col items-center -mt-20">
                                    <div className="w-full md:w-11/12 flex flex-col items-center rounded-xl md:border bg-card md:text-card-foreground md:shadow">
                                        <div className="flex flex-col gap-y-2 p-4">
                                            ...
                                            <p className="text-sm text-gray-600 mt-1 text-justify" dangerouslySetInnerHTML={{ __html: studio?.description || 'No description available.' }} />
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <Card className="w-full pt-6">
                                    <div className="p-6 pt-0 flex flex-col gap-y-4">
                                            ...
                                            {addon ? (
                                                addon.map((item)=>(
                                                    <AddonCard 
                                                        ...
                                                        title={item.name}
                                                        price={item.price}
                                                        quantity={addonQuantities.find(aq => aq.id === item.id)?.quantity || 0}
                                                        onQuantityChange={(quantity) => handleQuantityChange(item.id, quantity)}
                                                    />
                                                ))
                                            ):(
                                                <p className="text-gray-800">
                                                    {isLoading ? "Loading addons..." : "Tidak ada layanan tambahan yang tersedia."}
                                                </p>
                                            )}
                                        </div>
                                        ...
                                        <div className="flex gap-x-6 text-gray-600">
                                            ...
                                            <div className="flex gap-x-2">
                                                <Clock />
                                                <p className="text-gray-800">{convertBookingSession(bookingTime!)}</p>
                                            </div>
                                        </div>
                                        <hr className="border-t border-gray-200" />
                                        <p className="text-gray-500 text-sm">Voucher</p>
                                        <div className="flex gap-x-2">
                                            <Input type="text" placeholder="Masukan kode promo" value={selectedVoucher} onChange={(e) => setSelectedVoucher(e.target.value)} />
                                            <Button variant={"outline"} onClick={handleApplyVoucher}>{appliedVoucher ? 'Ganti Voucher' : 'Terapkan'}</Button>
                                        </div>
                                        {appliedVoucher && !isInvalidVoucher && (
                                            <div className="mt-2 text-sm text-green-600">
                                                Voucher {appliedVoucher.name} diterapkan ({appliedVoucher.discount}% discount)
                                            </div>
                                        )}
                                        {isInvalidVoucher && (
                                        ...
                                        <Button className="w-full" onClick={handleContinue}>Selanjutnya</Button>
                                    </div>
                                </Card>
                            </div>
                        </div>
